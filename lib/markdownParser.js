...
function parseMarkdown(file, tags, callback) {
    ...
    var lineTypes = {
        ...
        'TAG': {
            ...
            strip: function(line) {
                ...
                var line = this.match(line)[1].replace(regex, '');
                return line.split(',').map(function(tag) {
    ...
    function containsTags(needles, haystack) {
        if (haystack === false) {
            return;
        }
        var match = false;
    ...
    function handleLine(line, callback) {
        if (!output.skip) {
            ...
            switch (type) {
                case 'SPEC':
                    if (!tags.length || containsTags(tags, output.currentTag)) {
