...
function JunitXmlWriter(config) {

    this.writeResults = function(results) {
        var xml = builder.create('testsuite').att('tests', countTests(results));
        var testcases = [];
        var outputFile = (config && config['junit-out']) ? config['junit-out'] : 'junit-output.xml';
        testcases.push(parseTestCases(results.passed));
        testcases.push(parseTestCases(results.failed, {
            type: 'failure'
        }));
        testcases.push(parseTestCases(results.pending, {
            type: 'pending'
        }));
        ...
        fs.writeFileSync(outputFile, xml.end({
            pretty: true
        }));
    };
